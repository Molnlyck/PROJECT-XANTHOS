<?	
$topic = "SELECT * FROM topic WHERE semester='3' AND class='webudvikling'";
...
while($row = mysqli_fetch_assoc($getTopics)) {
	...
	while ($get = mysqli_fetch_assoc($getTitles)) {
		...
		if ($opgave_topic_id == $topic_id) {
			echo "<li><a href='https://skole.vibedrive.dk/semester/3/web/opgave.php?u=$opgave_id'>$opgave_title</a></li>";
		}
