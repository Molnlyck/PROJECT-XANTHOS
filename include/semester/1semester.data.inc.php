<?	
$topic = "SELECT * FROM topic WHERE semester='1' AND class='database'";
...
while($row = mysqli_fetch_assoc($getTopics)) {
	...
	while ($get = mysqli_fetch_assoc($getTitles)) {
		...
		if ($opgave_topic_id == $topic_id) {
			echo "<li><a href='https://skole.vibedrive.dk/semester/1/data/opgave.php?u=$opgave_id'>$opgave_title</a></li>";
		}
